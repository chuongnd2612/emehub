  ...
  return (
    <div className="flex flex-col gap-[14px]">
      <div className="grid grid-cols-4 gap-[14px]">
        <Kpi label="TICKETS MIRRORED">
          <KpiValue value={String(project.tickets)} />
        </Kpi>
        <Kpi label="AGENT RUNS">
          <KpiValue value={String(project.runs)} />
        </Kpi>
        <Kpi label="PASS RATE">
          <KpiValue value={project.passRate} className="text-ok" />
        </Kpi>
        <Kpi label="KNOWLEDGE CONFIDENCE">
          <div className="mt-[7px] flex items-baseline gap-[6px]">
            <span
              className={`text-[28px] font-black tracking-[-.04em] ${confidenceToneClass(
                project.confidence,
              )}`}
            >
              {project.confidence}
      ...
      <div className="grid grid-cols-4 gap-[14px]">
        ...
        <Meta label="PAGE OBJECTS">
          <div className="mt-[5px] text-[13px] font-bold text-txt2">
            {project.repository.pageObjects} reusable
          </div>
        </Meta>
      </div>
    </div>
