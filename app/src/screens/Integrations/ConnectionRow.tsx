...
// summary, status pill, `imported <when>`, trash button (hover → rose).
// Expanded: a 2-column field grid (text/password inputs, focus border --pb) +
// `Test connection` (spins ~1300 ms then toasts and marks the connection
// Connected / just now) + `Save connection` + "Credentials encrypted at rest".

import { Icon, Input, Spinner, StatusPill } from "@/components/ui";
import type { ProviderConnection } from "@/data";
        ...
        <div className="border-t border-bd3 px-[18px] pt-1 pb-[18px]">
          <div className="my-4 grid grid-cols-2 gap-[14px]">
            {connection.fields.map((f) => (
              <div key={f.key}>
                ...
                {/* Secret fields stay type="password" — never rendered in plaintext. */}
                <Input
                  type={f.type}
                  value={f.value}
          ...
          <div className="flex flex-wrap items-center gap-[10px]">
            ...
            <button
              ...
              onClick={onSave}
              className={cn(
                "cursor-pointer rounded-control-lg border border-bd2 bg-card3 px-4 py-[10px]",
                "text-[12.5px] font-semibold text-txt2 transition-colors duration-200 hover:bg-bd2",
              )}
            >
              Save connection
            </button>
