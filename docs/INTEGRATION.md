...
| `GET` | `/projects` | Project registry. Each row carries a `summary` of non-secret card figures (repo, branch, counts, knowledge status, `lastSyncedAt`) so a list screen costs one request, not 3N+1. **No test-account material, not even `hasPassword`.** `summary.provider` is the ticket source, resolved from the project's `work_item_connection_id`, with the knowledge row's `provider` as fallback — it used to be read from the knowledge row alone, which an agent writes, so a hub-configured project reported no source at all. `summary.lastSyncedAt` is `MAX(tickets.synced_at)` for that project, `null` when it has never synced. |
